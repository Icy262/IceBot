...
impl MCByte {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_i8()
			.expect("Should be able to read i8 from stream");
		Self { value: value }
	}
	...
	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_i8(data.value)
			.expect("Should be able to write i8 to stream");
	}
}
...
impl MCUByte {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_u8()
			.expect("Should be able to read u8 from stream");
		Self { value: value }
	}
	...
	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_u8(data.value)
			.expect("Should be able to write u8 to stream");
	}
}
...
impl MCShort {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_i16::<BigEndian>()
			.expect("Should be able to read i16 from stream");
		Self { value: value }
	}

	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_i16::<BigEndian>(data.value)
			.expect("Should be able to write i16 to stream");
	}
}
...
impl MCInt {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_i32::<BigEndian>()
			.expect("Should be able to read i32 from stream");
		Self { value: value }
	}

	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_i32::<BigEndian>(data.value)
			.expect("Should be able to write i32 to stream");
	}
}
...
impl MCLong {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_i64::<BigEndian>()
			.expect("Should be able to read i64 from stream");
		Self { value: value }
	}

	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_i64::<BigEndian>(data.value)
			.expect("Should be able to write i64 to stream");
	}
}
...
impl MCFloat {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_f32::<BigEndian>()
			.expect("Should be able to read f32 from stream");
		Self { value: value }
	}

	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_f32::<BigEndian>(data.value)
			.expect("Should be able to write f32 to stream");
	}
}
...
impl MCDouble {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		let value = stream
			.read_f64::<BigEndian>()
			.expect("Should be able to read f64 from stream");
		Self { value: value }
	}

	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_f64::<BigEndian>(data.value)
			.expect("Should be able to write f64 to stream");
	}
}
...
impl MCString8 {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		...
		stream
			...
			.expect("Should be able to read length u8 from stream");
		let text = String::from_utf8(buffer)
			.expect("Should be able to convert array of utf8 bytes to string");
		Self {
			length: length,
			text: text,
		}
	...
	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		...
		stream
			.write_all(data.text.as_bytes())
			.expect("Should be able to write all bytes of utf8 to stream");
	}
}
...
impl MCString16 {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		...
		stream
			.read_exact(&mut buffer)
			.expect("Should be able to read exact number of bytes");
		let text = String::from_utf16(
			&buffer
				.chunks_exact(2)
				.map(|character_bytes| u16::from_be_bytes([character_bytes[0], character_bytes[1]]))
				.collect::<Vec<u16>>(),
		)
		.expect("Should be valid UCS2 data");
		Self {
			length: length,
			text: text,
		}
...
impl MCBool {
	...
	pub(crate) fn write<W: Write>(stream: &mut W, data: Self) {
		stream
			.write_u8(data.value as u8)
			.expect("Should be able to write bool (u8) to stream");
	}
}
...
impl MCInventoryPayload {
	pub(crate) fn read<R: Read>(stream: &mut R) -> Self {
		...
		for _ in 0..count.value {
			payload.push(MCItem::read(stream))
		}
		Self {
			count: count,
			payload: payload,
